...
applicant_filter = st.selectbox("Filter by Applicant ID", ["All"] + all_applicants)
doc_type_filter = st.selectbox("Filter by Document Type", ["All"] + sorted(doc_df["predicted_classification"].unique().tolist()))
...
if not filtered_doc_df.empty:
    ...
    view_option = st.radio(
        "View Options", ["Raw Document", "Extracted Information"], key=f"view_{row.get('id', '')}"
    )
    ...
    with st.expander("💬 Officer Comments"):
        new_comment = st.text_area(
            ...
            key=f"comment_{row.get('id', '')}"
        )
    if st.button("💾 Save Updates", key=f"save_{row.get('id', '')}"):
        updated = row.copy()
...
if applicant_filter != "All":
    ...
    if not loan_app_row.empty:
        ...
        new_status = st.selectbox(
            ...
            key=f"loan_status_{loan_app_item.get('id', '')}"
        )
        if st.button("💾 Save Loan Application Status", key=f"save_loan_status_{loan_app_item.get('id', '')}"):
            # Update in Cosmos
